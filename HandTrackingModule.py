...
class handDetector:
    ...
    def fingersUP(self, img):
        ...
        if landmarkFound:
            ...
            if handDetector.isHandStraight(self, img, draw=True):
                ...
                # thumb
                fingersuplist.append(landMarkList[hc.INDEX_PIP][hc.y_axis] - landMarkList[hc.THUMb_IP][hc.y_axis]>45)

                if rightHand:

                    for tip in tipsList:
                        fingersuplist.append(landMarkList[tip][hc.x_axis] - landMarkList[tip - 2][hc.x_axis] > 0)
                else:
    ...
    def isHandStraight(self, img, draw=False):
        ...
        if landmarkFound:
            ...
            if landmarkList[hc.WRIST][hc.y_axis] > landmarkList[hc.THUMB_TIP][hc.y_axis]:

                # straight
                if landmarkList[hc.WRIST][hc.y_axis] > landmarkList[hc.PINKY_MCP][hc.y_axis]:
                    if draw:
...
def main():
    ...
    while True:
        ...
        # detector.landmarkDistance(img, 16, 20)
        # print(fingers)
        # print(checkHand)
